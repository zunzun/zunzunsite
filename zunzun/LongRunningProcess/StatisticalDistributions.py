import inspect, time, math, random, multiprocessing, cStringIO

import numpy, scipy, scipy.stats, pyeq2
...
def parallelWorkFunction(distributionName, data, sortCriteriaName):
    try:
        return pyeq2.Services.SolverService.SolverService().SolveStatisticalDistribution(distributionName, data, sortCriteriaName)
    except:
        return 0
...
class StatisticalDistributions(StatusMonitoredLongRunningProcessPage.StatusMonitoredLongRunningProcessPage):
    ...
    def PerformWorkInParallel(self):
        ...
        for i in self.completedWorkItemsList:
            ...
            # dig out a long name
            longName = cStringIO.StringIO(distro.__doc__).readlines()[0]
            if longName[:2] == 'A ':
                longName = longName[2:]
            if longName[:3] == 'An ':
                longName = longName[3:]
            ...
            try:
                ...
                i[1]['additionalInfo'] = cStringIO.StringIO(notes).readlines()
            except:
