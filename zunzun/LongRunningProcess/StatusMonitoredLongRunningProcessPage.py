import sys, os, time, multiprocessing, cStringIO, traceback, string, cPickle
from BeautifulSoup import BeautifulSoup # don't need everything, it has several components
...
import DataObject
import ClassForAttachingProperties
import ReportsAndGraphs
...
'''
    ...
    def CreateReportPDF(self):
        ...
        try:
            ...
            for report in self.textReports:
                ...
                soup = BeautifulSoup(joinedString, convertEntities=BeautifulSoup.HTML_ENTITIES)

                notUnicodeList = []
                for i in soup.findAll(text=True):
                    notUnicodeList.append(str(i))
                replacedText = string.join(notUnicodeList)
            ...
            for report in self.graphReports:
                ...
                if os.path.isfile(report.physicalFileLocation):
                    ...
                    pageElements.append(reportlab.platypus.XPreformatted('<br/><br/>', styles['CenteredBodyText']))
                    try:
                        im = reportlab.platypus.Image(cStringIO.StringIO(open(report.physicalFileLocation, 'rb').read()), self.dataObject.graphWidth * scale, self.dataObject.graphHeight * scale)
                    except:
                        time.sleep(1.0)
                        im = reportlab.platypus.Image(cStringIO.StringIO(open(report.physicalFileLocation, 'rb').read()), self.dataObject.graphWidth * scale, self.dataObject.graphHeight * scale)
                    im.hAlign = 'CENTER'
    ...
    def SaveDictionaryOfItemsToSessionStore(self, inSessionStoreName, inDictionary):
        if eval('self.session_' + inSessionStoreName) is None:
            exec('self.session_' + inSessionStoreName + ' = SessionStore(self.session_key_' + inSessionStoreName + ')')
        for i in list(inDictionary.keys()):
            item = inDictionary[i]
            if -1 != str(type(item)).find('unicode'):
                item = django.utils.encoding.smart_str(item, encoding='utf-8', strings_only=True, errors='strict')
            item = cPickle.dumps(item)
            
            exec('self.session_' + inSessionStoreName + '[i] = item')

        if inSessionStoreName == 'status':
            exec('self.session_' + inSessionStoreName + '["timestamp"] = cPickle.dumps(time.time())')

        # sometimes database is momentarily locked, so retry on exception to mitigate
        try:
            exec('self.session_' + inSessionStoreName + '.save()')
        except:
            time.sleep(0.5) # wait 1/3 second before retry
            exec('self.session_' + inSessionStoreName + '.save()')
            
        close_old_connections()
        exec('self.session_' + inSessionStoreName + ' = None')
    ...
    def LoadItemFromSessionStore(self, inSessionStoreName, inItemName):
        if eval('self.session_' + inSessionStoreName) is None:
            exec('self.session_' + inSessionStoreName + ' = SessionStore(self.session_key_' + inSessionStoreName + ')')
        try:
            exec('returnItem = self.session_' + inSessionStoreName + '[\'' + inItemName + '\']')
        except:
            returnItem = cPickle.dumps(None)
        close_old_connections()
        exec('self.session_' + inSessionStoreName + ' = None')
        
        returnItem = cPickle.loads(str(returnItem))
        return returnItem
