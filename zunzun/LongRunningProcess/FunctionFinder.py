...
class FunctionFinder(StatusMonitoredLongRunningProcessPage.StatusMonitoredLongRunningProcessPage):
    ...
    def AddEquationInfoToLinearAndParallelFittingListsAndCheckOneSecond(self):
        ...
        for i in self.dataObject.equation.GetDataCacheFunctions():
            if self.dictionaryOf_BothGoodAndBadCacheData_Flags.has_key(i[0]) != 1: # if not in the cached flags, add it
    ...
    def GenerateListOfWorkItems(self):
        ...
        if self.dataObject.dimensionality == 2:
            loopover = inspect.getmembers(pyeq2.Models_2D)
        else:
            loopover = inspect.getmembers(pyeq2.Models_3D)
            ...
            if inspect.ismodule(submodule[1]):
                ...
                for equationClass in inspect.getmembers(submodule[1]):
                    if inspect.isclass(equationClass[1]):
                        for extendedName in pyeq2.ExtendedVersionHandlers.extendedVersionHandlerNameList:
    ...
    def CreateUnboundInterfaceForm(self, request):
        ...
        dictionaryToReturn['header_text'] = 'ZunZunSite ' + str(self.dimensionality) + 'D Function Finder Interface'
        dictionaryToReturn['title_string'] = 'ZunZunSite ' + str(self.dimensionality) + 'D Function Finder Interface'
