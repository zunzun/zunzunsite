...
class CodeReportCPP(TextOnlyReport):
    ...
    def CreateReportOutput(self):
        code = pyeq2.outputSourceCodeService().GetOutputSourceCodeCPP(self.dataObject.equation)
...
class CodeReportFORTRAN90(TextOnlyReport):
    ...
    def CreateReportOutput(self):
        code = pyeq2.outputSourceCodeService().GetOutputSourceCodeFORTRAN90(self.dataObject.equation)
        if len(self.dataObject.equation.dataCache.allDataCacheDictionary['Weights']):
            code = code.replace("Fitting target value =", "Fitting target value (weighted) = ")
        self.stringList.append('<textarea rows="24" cols="85" wrap="OFF">' +  code + '</textarea>')
...
class CodeReportJAVA(TextOnlyReport):
    ...
    def CreateReportOutput(self):
        code = pyeq2.outputSourceCodeService().GetOutputSourceCodeJAVA(self.dataObject.equation)
        if len(self.dataObject.equation.dataCache.allDataCacheDictionary['Weights']):
            code = code.replace("Fitting target value =", "Fitting target value (weighted) = ")
        self.stringList.append('<textarea rows="24" cols="85" wrap="OFF">' +  code + '</textarea>')
...
class CodeReportJAVASCRIPT(TextOnlyReport):
    ...
    def CreateReportOutput(self):
        code = pyeq2.outputSourceCodeService().GetOutputSourceCodeJAVASCRIPT(self.dataObject.equation)
        if len(self.dataObject.equation.dataCache.allDataCacheDictionary['Weights']):
            code = code.replace("Fitting target value =", "Fitting target value (weighted) = ")
        self.stringList.append('<textarea rows="24" cols="85" wrap="OFF">' +  code + '</textarea>')
...
class CodeReportJULIA(TextOnlyReport):
    ...
    def CreateReportOutput(self):
        code = pyeq2.outputSourceCodeService().GetOutputSourceCodeJULIA(self.dataObject.equation)
        if len(self.dataObject.equation.dataCache.allDataCacheDictionary['Weights']):
            code = code.replace("Fitting target value =", "Fitting target value (weighted) = ")
        self.stringList.append('<textarea rows="24" cols="85" wrap="OFF">' +  code + '</textarea>')
...
class CodeReportPYTHON(TextOnlyReport):
    ...
    def CreateReportOutput(self):
        code = pyeq2.outputSourceCodeService().GetOutputSourceCodePYTHON(self.dataObject.equation)
        if len(self.dataObject.equation.dataCache.allDataCacheDictionary['Weights']):
            code = code.replace("Fitting target value =", "Fitting target value (weighted) = ")
        self.stringList.append('<textarea rows="24" cols="85" wrap="OFF">' +  code + '</textarea>')
...
class CodeReportCSHARP(TextOnlyReport):
    ...
    def CreateReportOutput(self):
        code = pyeq2.outputSourceCodeService().GetOutputSourceCodeCSHARP(self.dataObject.equation)
        if len(self.dataObject.equation.dataCache.allDataCacheDictionary['Weights']):
            code = code.replace("Fitting target value =", "Fitting target value (weighted) = ")
        self.stringList.append('<textarea rows="24" cols="85" wrap="OFF">' +  code + '</textarea>')
...
class CodeReportSCILAB(TextOnlyReport):
    ...
    def CreateReportOutput(self):
        code = pyeq2.outputSourceCodeService().GetOutputSourceCodeSCILAB(self.dataObject.equation)
        if len(self.dataObject.equation.dataCache.allDataCacheDictionary['Weights']):
            code = code.replace("Fitting target value =", "Fitting target value (weighted) = ")
        self.stringList.append('<textarea rows="24" cols="85" wrap="OFF">' +  code + '</textarea>')
...
class CodeReportMATLAB(TextOnlyReport):
    ...
    def CreateReportOutput(self):
        code = pyeq2.outputSourceCodeService().GetOutputSourceCodeMATLAB(self.dataObject.equation)
        if len(self.dataObject.equation.dataCache.allDataCacheDictionary['Weights']):
            code = code.replace("Fitting target value =", "Fitting target value (weighted) = ")
        self.stringList.append('<textarea rows="24" cols="85" wrap="OFF">' +  code + '</textarea>')
...
class CodeReportVBA(TextOnlyReport):
    ...
    def CreateReportOutput(self):
        code = pyeq2.outputSourceCodeService().GetOutputSourceCodeVBA(self.dataObject.equation)
        if len(self.dataObject.equation.dataCache.allDataCacheDictionary['Weights']):
            code = code.replace("Fitting target value =", "Fitting target value (weighted) = ")
        self.stringList.append('<textarea rows="24" cols="85" wrap="OFF">' +  code + '</textarea>')
...
class CoefficientAndFitStatistics(TextOnlyReport):
    ...
    def CreateReportOutput(self):
        ...
        for i in range(len(self.dataObject.equation.solvedCoefficients)):
            if eval('self.dataObject.equation.tstat_beta' + weightedString) == None:
                tstat = 'n/a'
            else:
                tstat = '%-.5E' %  (eval('self.dataObject.equation.tstat_beta' + weightedString)[i])

            if eval('self.dataObject.equation.pstat_beta' + weightedString) == None:
                pstat = 'n/a'
            else:
...
class ErrorListing(TextOnlyReport):
    ...
    def CreateReportOutput(self):
        ...
        breakLoop = False
        for i in reversed(range(dopIndep1)):
            if breakLoop == True:
                break
        ...
        breakLoop = False
        for i in reversed(range(dopDep)):
            if breakLoop == True:
                break
            for j in range(datalen): # number of data points
        ...
        if self.dataObject.dimensionality == 3:
            breakLoop = False
            for i in reversed(range(dopIndep2)):
                if breakLoop == True:
                    break
                for j in range(datalen): # number of data points
        ...
        if self.dataObject.dimensionality == 2:
            ...
            if self.dataObject.equation.modelRelativeError == None:
...
class RotationImageIterator:
    def __init__(self, inFig, inAx, inElevation):
        self.fig = inFig
        self.ax = inAx
        self.elevation = inElevation
        self.rotationAngle = 0
        self.endDegrees = 359
        self.step = 2 # animation frame every 2 degrees

    def __iter__(self):
        return self

    def next(self):
        if (self.rotationAngle + self.step) < self.endDegrees:
            self.rotationAngle += self.step
            self.ax.view_init(elev=self.elevation, azim=self.rotationAngle)
            return animation_tools.Figtodat.fig2img(self.fig)
        else:
            raise StopIteration()
...
class ScatterAnimation(GraphReport):
    ...
    def CreateCharacterizerOutput(self):
        ...
        try:
            [fig, ax, plt] = MatplotlibGraphs_3D.ScatterPlot3D(dataObject, None)
            
            imageIter = RotationImageIterator(fig, ax, dataObject.altimuth3D)
                        
            # display rate is 10 milliseconds for each 2 degrees
            animation_tools.images2gif.writeGif(self.physicalFileLocation,imageIter,duration=0.01,dither=0)
...
class SurfaceAnimation(GraphReport):
    ...
    def CreateReportOutput(self):
        try:
            ...
            [fig, ax, plt] = MatplotlibGraphs_3D.SurfacePlot(dataObject, None)            
