...
class UsesDataForm_BaseClass(django.forms.Form) :
    ...
    def LoadAndVerifyTextData(self):
        ...
        temp = django.utils.encoding.smart_str(self.cleaned_data["textDataEditor"], encoding='ascii', errors='ignore')        
        ...
        if 'commaConversion' not in self.cleaned_data.keys():
        ...
        self.equationBase = pyeq2.IModel.IModel()
        ...
        pyeq2.dataConvertorService().ConvertAndSortColumnarASCII(temp, self.equationBase, useWeightedFittingFlag)
...
class CharacterizeDataForm_1D (CharacterizeDataForm_BaseClass) :
    ...
    def clean_dataNameX(self):
        return django.utils.encoding.smart_str(self.cleaned_data['dataNameX'], encoding='ascii', errors='ignore')
...
class CharacterizeDataForm_2D (CharacterizeDataForm_1D) :
    ...
    def clean_dataNameY(self):
        return django.utils.encoding.smart_str(self.cleaned_data['dataNameY'], encoding='ascii', errors='ignore')
...
class CharacterizeDataForm_3D (CharacterizeDataForm_2D) :
    ...
    def clean_dataNameZ(self):
        return django.utils.encoding.smart_str(self.cleaned_data['dataNameZ'], encoding='ascii', errors='ignore')
...
class Equation_2D(CharacterizeDataForm_2D) :
    ...
    polyLength = max(len(pyeq2.PolyFunctions.GenerateListForPolyfunctionals_2D()), len(pyeq2.PolyFunctions.GenerateListForCustomPolynomials_2D()))
    ...
    for i in range(len(pyeq2.PolyFunctions.GenerateListForRationals_2D())):
...
class Equation_3D (CharacterizeDataForm_3D) :
    ...
    for i in range(len(pyeq2.PolyFunctions.GenerateListForPolyfunctionals_3D_X())):
        for j in range(len(pyeq2.PolyFunctions.GenerateListForPolyfunctionals_3D_Y())):
