...
import os, sys, time, urllib, signal, copy, cPickle
import forms, formConstants
import numpy, multiprocessing

import zunzun
import pyeq2
import LongRunningProcess
...
def StatusView(request):
    ...
    if session_status.has_key('redirectToResultsFileOrURL'):
        if cPickle.loads(str(session_status['redirectToResultsFileOrURL'])) != '':
            # read and reset
            redirect = cPickle.loads(str(session_status['redirectToResultsFileOrURL']))
            session_status['redirectToResultsFileOrURL'] = cPickle.dumps('')
    ...
    session_status['time_of_last_status_check'] = cPickle.dumps(time.time())
    try: # database can lock, sleep and retry
        session_status.save()
    except:
        time.sleep(0.5)
    ...
    try:
        currentStatus = cPickle.loads(str(session_status['currentStatus']))
        startTime = cPickle.loads(str(session_status['start_time']))
        timeStamp = cPickle.loads(str(session_status['timestamp']))
    except:
        return HttpResponse("I could not read your session data, my apologies. This is usually caused by a stale browser cookie. Please delete the zunzun.com browser cookie and try again.")
...
def LongRunningProcessView(request, inDimensionality, inEquationFamilyName='', inEquationName=''): # from urls.py, inDimensionality can only be '1', '2' or '3'
    ...
    LRP.inEquationName = urllib.unquote(inEquationName)
    LRP.inEquationFamilyName = urllib.unquote(inEquationFamilyName)
...
def FeedbackView(request):
    ...
    if request.method == 'POST':
        ...
        EmailMessage('ZunZun.com Feedback Form', msg, to = [settings.FEEDBACK_EMAIL_ADDRESS]).send()

        return render_to_response('zunzun/feedback_reply.html', {})
    else: # not a POST
        return HttpResponseRedirect('http://zunzun.com/')
...
def HomePageView(request):
    ...
    items_to_render = {}
    items_to_render['dim_to_spline_list'] = [['2',  pyeq2.Models_2D.Spline.Spline()], ['3',  pyeq2.Models_3D.Spline.Spline()]]
    items_to_render['dim_to_map_list'] = [['2', GetEquationInfoDictionary(2, 'Standard')], ['3', GetEquationInfoDictionary(3, 'Standard')]]
    items_to_render['header_text'] = 'ZunZun.com Online Curve Fitting<br>and Surface Fitting Web Site'
...
def AllEquationsView(request, inDimensionality, inAllOrStandardOnly): # from urls.py, inDimensionality can only be '2' or '3'
    ...
    if inAllOrStandardOnly == 'All':
        items_to_render['header_text'] = 'ZunZun.com List Of All ' + inDimensionality + 'D Equations'
    else:
        items_to_render['header_text'] = 'ZunZun.com List Of All Standard ' + inDimensionality + 'D Equations'
...
def GetEquationInfoDictionary(inDimensionality, inAllOrStandardOnly):
    ...
    if inDimensionality == 2:
        submodules = inspect.getmembers(pyeq2.Models_2D)
    else:
        submodules = inspect.getmembers(pyeq2.Models_3D)
    ...
    if inAllOrStandardOnly == 'Standard':
        ...
        extendedNameList = pyeq2.ExtendedVersionHandlers.extendedVersionHandlerNameList
                ...
                for extendedName in extendedNameList:
                    for equationClass in inspect.getmembers(submodule[1]):
                        if inspect.isclass(equationClass[1]):
                            ...
                            temp.url_quote_name = urllib.quote(temp.name)
    ...
    allEquationClassPropertiesList.sort(functionToSortListOfEquationPropertyClassesByName)
...
def functionToSortListOfEquationPropertyClassesByName(a,  b):
    # first sort on submodule name
    if a.submoduleName < b.submoduleName:
       return -1
    if a.submoduleName > b.submoduleName:
        return 1
        
    # submodule names must be the same, sort on extended name
    if a.extendedName != 'Default' and b.extendedName == 'Default': # empty strings first in sorted results
        return 1
    if a.extendedName == 'Default' and b.extendedName != 'Default': # empty strings first in sorted results
        return -1
    if a.extendedName != 'Offset' and b.extendedName == 'Offset': # Offset forms second in sorted results
        return 1
    if a.extendedName == 'Offset' and b.extendedName != 'Offset': # Offset forms second in sorted results
        return -1
    if a.extendedName < b.extendedName: # non-ofset extended forms last
       return -1
    if a.extendedName > b.extendedName:
        return 1
        
    # extended names must be the same, sort on name
    if a.name < b.name:
       return -1
    if a.name > b.name:
        return 1
