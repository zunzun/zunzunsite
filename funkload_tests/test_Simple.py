...
class Simple(FunkLoadTestCase):
    ...
    def RetrieveOnePage(self, url, assertString):
        ...
        time.sleep(0.5)
        self.assert_(assertString in str(self.getBody()), 'Could not load page ' + url)
...
#        time.sleep(0.5)
        ...
        if -1 != str(self.getBody()).find('This field is required'):
            self.assert_(0, 'Form is missing a required field')
        ...
        while (-1 != body.find('REDIRECT')) or (-1 != body.find('REFRESH')):
            if (time.time() - startTime) > inTimeoutSeconds:
                self.assert_(0, 'More than ' + str(inTimeoutSeconds) + ' seconds have passed for ' + inDescription + '\n\n' + body)
            time.sleep(3.0)
            self.get(self.server_url + '/StatusAndResults/', description='Getting status for ' + inDescription)
            time.sleep(0.5)
            body = str(self.getBody())
        ...
        for testString in inStringListForAssertions:
            self.assert_(-1 != body.find(testString), 'Incorrect ' + inDescription + ': "' + testString + '"')
    ...
    def test_simple(self):
        ...
        if testPolynomialQuadratic2D_SSQREL:
            ...
            self.assert_(-1 != str(self.getBody()).find('3.9510208'), 'Polynomial Quadratic 2D (SSQREL) - Evaluate At A Point')
        ...
        if testSpline2D:
            ...
            self.assert_(-1 != str(self.getBody()).find('4.02361487093'), 'Spline 2D - Evaluate At A Point')
        ...
        if testUserDefinedFunction2D:
            ...
            self.assert_(-1 != str(self.getBody()).find('4.1924427'), 'User Defined Function 2D (SSQABS) - Evaluate At A Point')
        ...
        if testPolynomialLinearWithExponentialDecay2D_SSQABS:
            ...
            self.assert_(-1 != str(self.getBody()).find('1.351027'), 'Polynomial Linear With Exponential Decay 2D (SSQABS) - Evaluate At A Point')
